...
/// turns a histogram into its corresponding huffman code tree
///
...
/// anti-memory-leak weapon. aim at root of the huffman code tree
///
/// recursive function that will clean up the huffman code tree structure after
/// you're done with it.
void cleanTree(node* n);

/// fills `map` based on the tree given by `root`
///
...
/// use `huffmap` to translates bytes of `fin` to codes in `fout`
///
...
/// use `huffmap` to translates huffman codes of `fin` to bytes in `fout`
///
